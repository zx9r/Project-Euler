...
def problem59():
    with open("p059_cipher.txt") as f:
        cypher = list(map(int, f.readline().split(",")))

    for password in get_passwords():
        plain = [c ^ password[i % len(password)] for i, c in enumerate(cypher)]
        text = ''.join(map(chr, plain))
        if " euler" in text.lower():
            print(f"Text: {text}")
            print("Password: {}".format(''.join(map(chr, password))))
            return sum(plain)
